<template>
  <div id="app">
    ...
    <!-- 唱片 -->
    <recordPlayer :songInfo='curSongInfo' :recorderStyle='isCD' />
    <!-- 唱片样式切换 -->
    <changeRecordStyleBtn :isCD='isCD' @changeFromBtn='changeStyle' />
    <!-- 控制按钮 -->
    <controller :songId='curSongId' @play="goPlay" />
    <!-- 按钮控制 -->
  </div>
</template>
<script>
    ...
    curSongIndex: function (val, oldVal) {
      if (!!val) {
        ...
        if (this.randomPlay) {
          ...
          this.curSongId = this.sondList[val].id
        }
      }
    }
  },
  methods: {
    ...
    creatRandomList() {
      ...
      this.shuffle([...this.sondList]).then((newArr) => {
        ...
        this.curSongId = this.sondList[newArr[this.curSongIndex]].id
      });
    },
    getNetData() {
      //从bird.ioliu.cn获取列表
